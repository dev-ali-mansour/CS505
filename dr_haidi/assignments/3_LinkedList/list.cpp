...
void list::updateData(const student &d) {
    cursor->data = d;
}
...
void list::retrieveData(student &d) const {
    d = cursor->data;
}
...
void list::insertFirst(const int &k, const student &d) {
    toFirst();
...
void list::insertAfter(const int &k, const student &d) {
...
void list::insertBefore(const int &k, const student &d) {
...
void list::insertEnd(const int &k, const student &d) {
...
void list::orderInsert(const int &k, const student &d) {
    toFirst();
...
void list::traverse() {
    ...
    while (!cursorIsEmpty()) {
        ...
        cout << "id: " << cursor->data.st_no << endl;
        cout << "name: " << cursor->data.st_name << endl;
        cout << "grade: " << cursor->data.st_grade << endl;
        cout << "===================================" << endl;
        advance();
    }
}

float list::getGradesAverage() {
    if (listIsEmpty())return 0;
    int count = 0;
    float sum = 0;
    toFirst();
    while (!cursorIsEmpty()) {
...
//    return sum / listSize(); //using listSize() makes another loop through the list
    return sum / count;
}